...
parte del proyecto. Reemplazar estas llamadas por la función
eliminar_seguro() de automatizacion/utils/seguridad_archivos.py.
Esta función verifica si la carpeta contiene archivos críticos (.json, .py, .db)
y los mueve a basurero_temporal/ en lugar de borrarlos del disco.
...
en el código. Implementar pathlib.Path(__file__).parent para que todas
las rutas sean relativas a la raíz del proyecto. El software debe ser
portable a cualquier servidor (Italia, Estonia o Argentina) sin cambios.
