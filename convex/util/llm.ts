...
const VERTEX_EMBEDDING_DIMENSION = 768;

export const EMBEDDING_DIMENSION: number = VERTEX_EMBEDDING_DIMENSION;

export function detectMismatchedLLMProvider() {
  if (process.env.LLM_PROVIDER === 'vertex' || process.env.GOOGLE_CLOUD_PROJECT) {
    return;
  }
...
export function getLLMConfig(): LLMConfig {
  // Vertex AI (Google Cloud) — QuantumHive default
  if (process.env.LLM_PROVIDER === 'vertex' || process.env.GOOGLE_CLOUD_PROJECT) {
    ...
    return {
      ...
      chatModel: process.env.LLM_MODEL ?? 'gemini-2.5-flash',
