...
logger.info("=" * 60)
logger.info("TEST DE CONEXION BINANCE TESTNET")
logger.info("=" * 60)
...
if balance_data:
    ...
    logger.error("  1. Las API keys expiraron — regenerar en https://testnet.binancefuture.com/")
    logger.error("  2. Las keys no tienen permisos de USD-M Futures (Futuros)")
    logger.error("  3. IP restriction activada — desactivar o añadir 0.0.0.0/0")
    logger.error("  4. BASE_URL incorrecta (debe ser https://testnet.binancefuture.com)")
    logger.error("")
    logger.error("COMO REGENERAR:")
    logger.error("  1. Ir a https://testnet.binancefuture.com/")
    logger.error("  2. Iniciar sesion con GitHub o Google")
    logger.error("  3. Si no tienes cuenta: registrarte (toma 1 minuto)")
    logger.error("  4. Ir a API Management → Create API Key")
    logger.error("  5. DESACTIVAR IP restriction (dejar en blanco) o poner 0.0.0.0/0")
    logger.error("  6. Habilitar USD-M Futures en los permisos")
    logger.error("  7. Copiar API Key y Secret Key")
    logger.error("  8. Actualizar .env:")
    logger.error("       BINANCE_TESTNET_API_KEY=key_nueva")
    logger.error("       BINANCE_TESTNET_SECRET=secret_nuevo")
    logger.error("       BINANCE_TESTNET=false")
    logger.error("  9. Ejecutar este test de nuevo: python test_binance_conexion.py")
